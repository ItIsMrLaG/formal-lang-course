...
try:
    from project.task6_grammar_transform import hellings_based_cfpq
except ImportError:
...
def test_cfg_to_weak_normal_form_exists():
    try:
        import project.task6_grammar_transform

        assert "cfg_to_weak_normal_form" in dir(project.task6_grammar_transform)
    except NameError:
        assert False
