...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "wc", PATH_TO_RES_DIR_DATASET / "wc_graph_meta_result.json", id="wc"
        ),
        pytest.param(
            "atom", PATH_TO_RES_DIR_DATASET / "atom_graph_meta_result.json", id="atom"
        ),
        pytest.param(
            "core", PATH_TO_RES_DIR_DATASET / "core_graph_meta_result.json", id="core"
        ),
    ],
)
def test_get_graph_meta_by_name_on_existing_in_dataset_graphs(
    name: str, expected_as_json: Path
):
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            (2, 2),
            ("fstG", "sndG"),
            PATH_TO_RES_DIR / "two_cycles_labeled_small_graph.dot",
            id="small graph",
        ),
        pytest.param(
            (4, 5),
            ("fstG", "sndG"),
            PATH_TO_RES_DIR / "two_cycles_labeled_medium_graph.dot",
            id="medium graph",
        ),
    ],
)
def test_save_labeled_two_cycles_graph_to_dot(
    ...
    assert nx.is_isomorphic(
        expected_graph, result_graph, edge_match=dict.__eq__, node_match=dict.__eq__
    )
